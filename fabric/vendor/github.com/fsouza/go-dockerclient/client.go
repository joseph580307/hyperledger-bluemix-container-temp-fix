...
var (
    ...
    //set logger
    clientLogger = logging.MustGetLogger("clientcontroller")
)
...
func (c *Client) stream(method, path string, streamOptions streamOptions) error {
    ...
    clientLogger.Debugf("Protocol: %s",protocol)
    clientLogger.Debugf("Address: %s",address)
    ...
    clientLogger.Debugf("I'm here !!-- Client/Stream/flag 1'")

	if protocol == unixProtocol || protocol == namedPipeProtocol {
   clientLogger.Debugf("I'm here !!-- Client/Stream/flag 2'")
		var dial net.Conn
        ...
        clientLogger.Debugf("subCtx: %s" , subCtx)
        clientLogger.Debugf("HttPClient: %s",c.HTTPClient)
	...
	defer resp.Body.Close()
    clientLogger.Debugf("Response Status Code: %s",resp.StatusCode)
    clientLogger.Debugf("Response Body: %s",resp.Body)
	if resp.StatusCode < 200 || resp.StatusCode >= 400 {
		return newError(resp)
	}
   clientLogger.Debugf("I'm here !!-- Client/Stream/flag 5'")
	var canceled uint32
	if streamOptions.inactivityTimeout > 0 {
		ch := handleInactivityTimeout(&streamOptions, cancelRequest, &canceled)
		defer close(ch)
	}
   clientLogger.Debugf("I'm here !!-- Client/Stream/flag 6'")
   ...
   clientLogger.Debugf("I'm here !!-- Client/Stream/flag 7'")
	return nil
}
